 ...
 * @returns {{slug: string}[]}
 */
export function getAllArticles(bookSlug) {
 ...
 *   card: string,
 *   problem: string,
 *   neighborsPage: { prePage: object|null, nextPage: object|null }
 * }}
 */
export function getArticleBySlug(bookSlug, articleSlug) {
    const mdDir = path.join(process.cwd(), `public/books/${bookSlug}/md`);
    ...
    const mdRaw = fs.readFileSync(path.join(mdDir, `${articleSlug}.md`), "utf8");
